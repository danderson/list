...
type ErrDuplicateWildcardException struct {
	Name      string
	*Wildcard // Suffix or Wildcard
}

func (e ErrDuplicateWildcardException) Error() string {
	return fmt.Sprintf("%s: duplicate wildcard exception %q", e.LocationString(), e.Name)
}
